 ...
 * stays petId-agnostic (Step 2B-M2, extended Step 2B-M4/M5). Real, unmocked
...
/** Unique to FallbackCompanionArt's silhouette (any pet without its own presentation seam). */
...
describe("ChallengeStageShell companion rendering (CompanionActor migration)", () => {
  ...
  it("stays petId-agnostic: a deliberately-unconfigured petId renders the generic fallback companion, not Burpee art", () => {
    const { container } = render(
      <ChallengeStageShell
        worldId="world-unknown-for-test"
        title="Desafio"
        onClose={() => {}}
        petId="unconfigured-pet"
        narrationStatus="idle"
      >
        <p>conteúdo</p>
      </ChallengeStageShell>,
  ...
  it("renders Lyra (not the generic fallback) when petId is lyra", () => {
    const { container } = render(
      <ChallengeStageShell
        worldId="world-unknown-for-test"
        title="Desafio"
  ...
  it("frames Lyra in LYRA_BOX.viewBox, not Burpee's/anyone else's box", () => {
    const { container } = render(
      <ChallengeStageShell
        worldId="world-unknown-for-test"
        title="Desafio"
    ...
    const viewBox = companionSvg(container)?.getAttribute("viewBox");
    expect(viewBox).toBe(LYRA_BOX.viewBox);
    expect(viewBox).not.toBe(BURPEE_BOX.viewBox);
    expect(viewBox).not.toBe(PIPOCA_BOX.viewBox);
    expect(viewBox).not.toBe(WILL_BOX.viewBox);
  });
